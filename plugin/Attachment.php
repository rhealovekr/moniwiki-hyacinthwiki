...
# FIXME attachment:"hello.png" => error
  ...
  if (!empty($file_ok)) {
    ...
    if (empty($img_link) && preg_match("/\.(png|gif|jpeg|jpg|bmp)$/i",$upload_file, $m)) {
      ...
      // 이미지 사이즈 읽어서 가로 670px 이상이면 원본 링크 추가 -- yhyacinth 2014/02/22
      $resized = 0;
      $image_local = $_SERVER["DOCUMENT_ROOT"]."/moniwiki/".$_l_upload_file;

      if (file_exists($image_local))
        list($width, $height, $type, $image_attr) = getimagesize($image_local);
      else
      ...
      // 이미지에 <a href="원본 주소"></a> 추가 -- yhyacinth 2014/02/21
      ...
      if (!empty($img_link))
        return $bra."<span aa class=\"attach\"><a href='$link'>$img_link</a></span>".$ket;
