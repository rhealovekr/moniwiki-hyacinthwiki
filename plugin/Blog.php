...
//
// reply CAPTCHA 추가 -- hyacinth 2011/03/29 17:02:04
...
function do_Blog($formatter,$options) {
  ...
  $homepage = (!empty($options['homepage']) && strlen($options['homepage']) > 8) ? $options['homepage'] : '';
  if (preg_match('/[<>]/', $homepage))
    $homepage = '';
  else if (!preg_match('/^http:\/\//', $homepage))
    $homepage = '';
...
#add comment 문구 제거 -- rhealove 2012/12/04
    ...
    ### comment 제거 함 2014/04/02 rhealove
    if (!empty($options['value'])) {
      if (!empty($options['value'])) {
    ...
    if (empty($options['value'])) {
      ...
      $myinput='<div class="blog-action" style="text-align: left;"><span class="bullet"><font color="#125FAB"></font></span></div><br />';
...
#      $myinput.='<b>'._("Name")."</b>: <input name='name' size='15' maxlength='15' value='$name' />\n";
        ...
        $save_msg = _("Save");
## Blog에선 Preview 삭제 -- rhealove 2012/08/29
