...
#  $blog_name= urlencode($DBInfo->sitename.":$options[id]");
  ...
  ### 블로그 아닌 경우 구분 hyacinth 2013/04/02
  if (empty($options[value]))
    $url= $formatter->link_url($options['page']);
  else
    $url= $formatter->link_url($options['page'],"?action=blog&amp;value=$options[value]");
