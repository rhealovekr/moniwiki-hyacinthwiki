...
class WikiDB {
  ...
  ### get (fake) creation time 페이지 생성일 2014/02/20 rhealove
    ...
    if (($p = strpos($urltest, ':')) !== false and
      ...
      if (preg_match("/^(http|https|ftp)/",$url)) {
        ...
        if (preg_match("/(^.*\.(png|gif|jpeg|jpg))(?:\?|&(?!>amp;))?(.*?)?$/i", $url1, $match)) {
          ...
          ### 이미지에 <a href="원본"></a> 붙임 2014/02/21 rhealove
          global $DBInfo;
...
#    $depth--;
  ...
  function _table($on,&$attr) {
    ...
    return "<table style=\"border-collapse: separate; border-spacing:0\" $my>\n";
  }
  ...
  function send_page($body="",$options=array()) {
	...
	$this->postambles();
      ...
      if (!empty($body)) {
        ### XXX irc color hack 2014/03/17
        if ($this->page->name == "irc logs" || 
            $this->page->name == "test2")
        {
...
#          $body=preg_replace("//","",$body);
          ...
          $body=preg_replace("/12(.*)\n/","[[HTML(<font color='blue'>)]]$1 [[HTML(</font>)]]\n",$body);
          $body=preg_replace("/13(.*)\n/","[[HTML(<font color='#FF00FF'>)]]$1 [[HTML(</font>)]]\n",$body);
          $body=preg_replace("/14(.*)\n/","[[HTML(<font color='#808080'>)]]$1 [[HTML(</font>)]]\n",$body);
          $body=preg_replace("/15(.*)\n/","[[HTML(<font color='#D0D4D0'>)]]$1 [[HTML(</font>)]]\n",$body);
          $body=preg_replace("/0?1([^\d].*)\n/","[[HTML(<font color='black'>)]]$1 [[HTML(</font>)]]\n",$body);
          $body=preg_replace("/0?2(.*)\n/","[[HTML(<font color='#000080'>)]]$1 [[HTML(</font>)]]\n",$body);
          $body=preg_replace("/0?3(.*)\n/","[[HTML(<font color='#009400'>)]]$1 [[HTML(</font>)]]\n",$body);
          $body=preg_replace("/0?4(.*)\n/","[[HTML(<font color='red'>)]]$1 [[HTML(</font>)]]\n",$body);
          $body=preg_replace("/0?5(.*)\n/","[[HTML(<font color='#800000'>)]]$1 [[HTML(</font>)]]\n",$body);
          $body=preg_replace("/0?6(.*)\n/","[[HTML(<font color='#A000A0'>)]]$1 [[HTML(</font>)]]\n",$body);
          $body=preg_replace("/0?7(.*)\n/","[[HTML(<font color='#FF8000'>)]]$1 [[HTML(</font>)]]\n",$body);
          $body=preg_replace("/0?8(.*)\n/","[[HTML(<font color='yellow'>)]]$1 [[HTML(</font>)]]\n",$body);
          $body=preg_replace("/0?9(.*)\n/","[[HTML(<font color='#00FC00'>)]]$1 [[HTML(</font>)]]\n",$body);
          $body=preg_replace("/\d?\d/","",$body);
          $body=preg_replace("/||/","",$body);
        }
        // strtr old wiki markups
        if (!empty($this->trtags))
    ...
    for (; $ii < $lcount; $ii++) {
      ...
      if (preg_match('/^[ ]*(-{4,})$/',$line, $m)) {
        ### Category 이전 ---- 의 처리 -- rhealove 2014/04/08
        $nextline = $lines[$ii+1];
        ...
        ### Keyword인데 Category가 없으면 ---- 입력 rhealove 2014/04/17
        if ($keyword_found == 1 && $ii+1 == $lcount) {
          if (!preg_match('/^(\[|)Category.[^\]]*(\]|)$/', $nextline))
          $line = "[[HTML(<hr class=\"category_hr\">)]]" . $line;
        }
        ### Category를 tags 형식으로 변경 -- rhealove 2012/12/27
        if (preg_match('/^(\[|)(Category.[^\]]*)(\]|)$/', $line, $matches, PREG_OFFSET_CAPTURE)) {
          if (strcmp($line, 'CategoryCategory') == 0)
            $category = 'Category';
          else
            $category = str_replace('Category', '', $matches[2][0]); 

            $line = str_replace($line, '[[HTML(<div class=\'cloudView\'><a href="/moniwiki/wiki.php/CategoryCategory"><span class="meta_categories" title="Category"></span></a></div><div class=\'cloudView\'><ul><li class="tag-item"><a href="/moniwiki/wiki.php/'.$matches[2][0].'">'.$category.'</a></li></ul></div>)]]', $line);
        }
...
#     $close.=$this->_list(0,$indent_type[$in_li]);
  ...
  function send_header($header="",$options=array()) {
    ...
    if (!$plain) {
      ...
      $keywords = '';
      ### 사용자 키워드 추가 2013/11/25 rhealove
      if (!empty($this->pi['#user-keywords'])) {
        if (!empty($this->pi['#keywords']))
          ...
          $keywords='<meta name="keywords" content="'.$this->pi['#user-keywords'].'" />'."\n";
      }
      else if (!empty($this->pi['#keywords'])) {
      ...
      if ($options['action'] == "blog" or preg_match('/^Blog.*/',$options['pagename'])) {
        $relblogrss = '<link rel="alternate" type="application/rss+xml" title="hrp Atom Feed" href="http://feeds.feedburner.com/HrpBlog" />'."\n";
      }

      ### 블로그인 경우 본문 내용의 제목을 파싱해서 타이틀 설정 -- rhealove 2012/12/12
...
if (1) {
          ...
          $append=str_replace("||", "", $append);
          $append=str_replace("==", "", $append);
          $append=preg_replace("/wiki:.[^ ]* /", "", $append);
...
#        echo "<!--$append-->";
        ...
        ### #width xxx 스킨 가로 크기 지정 2013/11/25 rhealove
  ...
  div.entry-header-wrapper { height:20px; }
      ...
      ### find image -- rhealove 2013/11/08
      $blog_flag=0;
      foreach ($templines as $line) {
        if (preg_match('/^##/',$line))
          continue;
        ...
        if (preg_match("/(^http:\/\/.[^\?]*\.(bmp|jpg|jpeg|gif|png))/i",$line, $matches)) {
          $temp = "$matches[1]";
          array_push($images, $temp);
        }
        if (preg_match("/attachment:(.*\.(bmp|jpg|jpeg|gif|png))/i",$line, $matches)) {
          $temp = "http://".$_SERVER['HTTP_HOST'].$_SERVER['SCRIPT_NAME'].str_replace('%2F','/',rawurlencode($_SERVER['PATH_INFO']))."?action=download&value=$matches[1]";
          array_push($images, $temp);
        }
      }
      #if (empty($image)) {
      #  $image="http://lua.kr/images/1/2beba805723160cb02e5ca70460cc9bebd0e097f.jpg";
      #}
      #echo "<!--" . $image ."-->";
      ...
      if (empty($options['title'])) {
        ...
        $options['title'] = trim(strip_tags($options['title']));
      }
#      print_r($images);
      ...
      if (mb_strlen($description, 'utf-8') == 0 ||
          mb_strcmp($description, "페이지가 없습니다") == 0 ||
          strcmp($description, "File does not exist") == 0 ||
          strcmp($description, "No page found") == 0 ||
          strcmp($description, "Page does not exists") == 0) $og .= '<meta name="robots" content="noindex,nofollow" />'."\n";
      $og .= "<meta property=\"og:url\" content=\"http://" . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'] . "\" />\n";
      $og .= "<meta property=\"og:site_name\" content=\"hrp\" />\n";

      $twitter_card_image="";
      $og_image_cnt = 0;
      foreach ($images as $image) {
        if (preg_match('/^http:\/\/'.$_SERVER["HTTP_HOST"].'/',$image)) {
          #image size
          # ex) http://www.foo.com -> home1/foo/public_html
          $image_local = str_replace("http://".$_SERVER["HTTP_HOST"], $_SERVER["DOCUMENT_ROOT"], $image);
          # XXX hack
          if (preg_match('/moniwiki\/wiki\.php\/(.[^\?]*)/',$image,$match)) {
            $page_name = $match[1];
            $file_name = substr($image, 0 - (strlen($image) - strpos($image, 'value=') - 6));

            # XXX encode
            $page_name = preg_replace_callback('/%(\d[A-F]|[A-F]\d|[A-F][A-F])/', 'preg_replace_callback_lowercase', $page_name);
            $page_name = str_replace('%', '_', $page_name);
            $page_name = str_replace('/', '_2f', $page_name);
            $page_name = str_replace('-', '_2d', $page_name);

            $image_local = $_SERVER["DOCUMENT_ROOT"]."/moniwiki/pds/$page_name/$file_name";
          }

          $decoded = urldecode($image_local);
          if (is_file($decoded)) {
            list($width, $height, $type, $attr) = getimagesize($decoded);
          }
          ...
          if ($width > 3000 || $height > 3000) {
            ...
            $og .= "<meta property=\"og:image:type\" content=\"" . image_type_to_mime_type($type) . "\" />\n";
            $og .= "<meta property=\"og:image:width\" content=\"" . $width . "\" />\n";
            $og .= "<meta property=\"og:image:height\" content=\"" . $height . "\" />\n";
            ++$og_image_cnt;
          ...
          $og .= "<meta property=\"og:image:height\" content=\"360\" />\n";
          ++$og_image_cnt;
      ...
      $twitter_card .= "<meta name=\"twitter:creator\" content=\"yhyacinth\" />\n";

      $twitter_card = str_replace("&","&amp;",$twitter_card);
...
JSHEAD;
  ...
  function get_actions($args='',$options) {
    ...
    $menu[]=$this->link_tag("FindPage","",_("FindPage")," rel='nofollow'");
  ...
  function send_footer($args='',$options=array()) {
    ...
    if ($crtime=$DBInfo->crtime($options['page'])) {
      ...
      $crtime=date("g:i a, F j, Y", strtotime($crdit. " " . $crtime));
    }
...
FOOT;
  ...
  function send_title($msgtitle="", $link="", $options="") {
    ...
    ### 블로그 제목 페이지 이름 설정 -- 2014-01/16 rhealove
    ...
    if (!empty($link)) {
      ...
      if (strrpos($title, "/") > 0) {
        ...
        foreach ($sub_titles as $sub) {
          ...
          $sub_append="$sub_append/".rawurlencode($sub);
          $link=$_SERVER["HTTP_HOST"].$_SERVER["SCRIPT_NAME"]."$sub_append";
          if (count($sub_titles)==$cnt)
            $append=$append."<a href=\"http://$link?action=fullsearch$qext&amp;value="._urlencode($mypgname)."\">$slash$sub</a>";
          else
            $append=$append."<a href=\"http://$link\">$slash$sub</a>";
        }
        $title=$append;
      }
      else {
...
#        $menu[]=$this->link_tag($item,"",_($item),$attr);
    ...
    if (empty($this->css_friendly)) {
      ...
      foreach ($menu as $k=>$v) {
        ...
        ### XXX 메뉴 이름 변경 2014/02/22 rhealove
        if (1){
        if (preg_match('/>hyacinth</',$menu[$k])) {
          $menu[$k]=str_replace('>hyacinth<','>대하여<',$menu[$k]);
        } else if (preg_match('/>Blog</',$menu[$k])) {
          $menu[$k]=str_replace('>Blog<','>블로그<',$menu[$k]);
          $menu[$k]=str_replace('rel=\'nofollow\'','',$menu[$k]);
        } else if (preg_match('/>Lens</',$menu[$k])) {
...
#        echo '<body itemscope itemtype="http://schema.org/WebPage"'.(!empty($options['attr']) ? ' ' . $options['attr'] : '' ) .">\n";
        echo '<body'.(!empty($options['attr']) ? ' ' . $options['attr'] : '' ) .">\n";
      #echo '<div><a id="top" accesskey="t"></a></div>'."\n";
    }
    #
...
function wiki_main($options) {
  ...
  while (empty($action) or $action=='show') {
    ...
    ### &redirect robot noindex 2013/11/11 rhealove
  ...
  if ($action) {
    if (strcmp($action, "show") != 0 &&
      strcmp($action, "blog") != 0) {
      $options['metatags']="<meta name=\"robots\" content=\"noindex,nofollow\" />\n";
    }
    else {
      $options['metatags']="<meta name=\"robots\" content=\"index,follow\" />\n";
    }
    $options['custom']='';
    $options['help']='';
    $options['value']=$value;
